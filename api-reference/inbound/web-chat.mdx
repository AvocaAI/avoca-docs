...
api: 'POST /web-chat/sessions'
description: 'Initialize a new web chat session for your website'
---

<Note>
  Create AI-powered chat sessions for website visitors. The chat widget can be embedded on any page and provides real-time communication with intelligent responses.
</Note>

## Request

<ParamField body="website_id" type="string" required>
  Your website identifier from Avoca dashboard
</ParamField>
...
<ParamField body="custom_fields" type="object">
  Additional visitor data from your website
...
<ParamField body="assistant_id" type="string">
  Specific AI assistant to use (defaults to primary web chat assistant)
</ParamField>
...
```bash cURL
curl -X POST https://api.avoca.ai/v1/web-chat/sessions \
...
const session = await client.webChat.createSession({
...
# Create chat session
session = client.web_chat.create_session(
    website_id='site_abc123',
    visitor={
